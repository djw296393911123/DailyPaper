...
public class RxUtil {
     ...
     * @return
     */
    public static <T> Observable.Transformer<GankBaseReponse<T>, T> handleResult() {   //compose判断结果
        return new Observable.Transformer<GankBaseReponse<T>, T>() {
            @Override
            public Observable<T> call(Observable<GankBaseReponse<T>> httpResponseObservable) {
                return httpResponseObservable.flatMap(new Func1<GankBaseReponse<T>, Observable<T>>() {
                    @Override
                    public Observable<T> call(GankBaseReponse<T> tGankHttpResponse) {
